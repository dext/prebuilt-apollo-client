...
function shouldInclude(_a, variables) {
    ...
    return getInclusionDirectives(directives).every(function (_a) {
        ...
        if (ifArgument.value.kind === 'Variable') {
            ...
            tsInvariant.invariant(evaledValue !== void 0, "Invalid variable referenced in @" + directive.name.value + " directive.");
        }
        else {
...
function getInclusionDirectives(directives) {
    ...
    if (directives && directives.length) {
        directives.forEach(function (directive) {
            ...
            tsInvariant.invariant(directiveArguments && directiveArguments.length === 1, "Incorrect number of arguments for the @" + directiveName + " directive.");
            var ifArgument = directiveArguments[0];
            tsInvariant.invariant(ifArgument.name && ifArgument.name.value === 'if', "Invalid argument for the @" + directiveName + " directive.");
            var ifValue = ifArgument.value;
            tsInvariant.invariant(ifValue &&
...
function getFragmentQueryDocument(document, fragmentName) {
    ...
    document.definitions.forEach(function (definition) {
        if (definition.kind === 'OperationDefinition') {
            throw new tsInvariant.InvariantError("Found a " + definition.operation + " operation" + (definition.name ? " named '" + definition.name.value + "'" : '') + ". " +
    ...
    if (typeof actualFragmentName === 'undefined') {
        tsInvariant.invariant(fragments.length === 1, "Found " + fragments.length + " fragments. `fragmentName` must be provided when there is not exactly 1 fragment.");
...
function getFragmentFromSelection(selection, fragmentMap) {
    switch (selection.kind) {
        ...
        case 'FragmentSpread': {
            ...
            tsInvariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
            return fragment;
        }
        default:
            return null;
...
function valueToObjectRepresentation(argObj, name, value, variables) {
    if (isIntValue(value) || isFloatValue(value)) {
        ...
        throw new tsInvariant.InvariantError("The inline argument \"" + name.value + "\" of kind \"" + value.kind + "\"" +
...
function checkDocument(doc) {
    tsInvariant.invariant(doc && doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    var operations = doc.definitions
        .filter(function (d) { return d.kind !== 'FragmentDefinition'; })
        .map(function (definition) {
        if (definition.kind !== 'OperationDefinition') {
            throw new tsInvariant.InvariantError("Schema type definitions not allowed in queries. Found: \"" + definition.kind + "\"");
        }
        return definition;
    });
    tsInvariant.invariant(operations.length <= 1, "Ambiguous GraphQL document: contains " + operations.length + " operations");
    return doc;
}
...
function getQueryDefinition(doc) {
    ...
    tsInvariant.invariant(queryDef && queryDef.operation === 'query', 'Must contain a query definition.');
    return queryDef;
}
function getFragmentDefinition(doc) {
    tsInvariant.invariant(doc.kind === 'Document', "Expecting a parsed GraphQL document. Perhaps you need to wrap the query string in a \"gql\" tag? http://docs.apollostack.com/apollo-client/core.html#gql");
    tsInvariant.invariant(doc.definitions.length <= 1, 'Fragment must have exactly one definition.');
    var fragmentDef = doc.definitions[0];
    tsInvariant.invariant(fragmentDef.kind === 'FragmentDefinition', 'Must be a fragment definition.');
    return fragmentDef;
}
function getMainDefinition(queryDoc) {
    ...
    throw new tsInvariant.InvariantError('Expected a parsed GraphQL query with a query, mutation, subscription, or a fragment.');
}
    ...
    test: function (directive) {
        ...
        if (willRemove) {
            if (!directive.arguments ||
                ...
                tsInvariant.invariant.warn('Removing an @connection directive even though it does not have a key. ' +
...
function toPromise(observable) {
    ...
    return new Promise(function (resolve, reject) {
        observable.subscribe({
            next: function (data) {
                if (completed) {
                    tsInvariant.invariant.warn("Promise Wrapper does not support multiple results from Observable");
                }
                else {
                    completed = true;
                    resolve(data);
...
function validateOperation(operation) {
    ...
    for (var _i = 0, _a = Object.keys(operation); _i < _a.length; _i++) {
        ...
        if (OPERATION_FIELDS.indexOf(key) < 0) {
            throw new tsInvariant.InvariantError("illegal argument: " + key);
        }
    }
    return operation;
}
...
var ApolloLink = (function () {
    ...
    ApolloLink.concat = function (first, second) {
        ...
        if (isTerminating(firstLink)) {
            tsInvariant.invariant.warn(new LinkError("You are calling concat on a terminating link, which will have no effect", firstLink));
            return firstLink;
        }
    ...
    ApolloLink.prototype.request = function (operation, forward) {
        throw new tsInvariant.InvariantError('request is not implemented');
    };
...
var Reobserver = (function () {
    ...
    Reobserver.prototype.updatePolling = function () {
        ...
        tsInvariant.invariant(pollInterval, 'Attempted to start a polling query without a polling interval.');
        ...
        return this.queryManager.fetchQuery(qid, combinedOptions, exports.NetworkStatus.fetchMore).then(function (fetchMoreResult) {
            ...
            if (updateQuery) {
                if (process.env.NODE_ENV !== "production" &&
                    ...
                    tsInvariant.invariant.warn("The updateQuery callback for fetchMore is deprecated, and will be removed\nin the next major version of Apollo Client.\n\nPlease convert updateQuery functions to field policies with appropriate\nread and merge functions, or use/adapt a helper function (such as\nconcatPagination, offsetLimitPagination, or relayStylePagination) from\n@apollo/client/utilities.\n\nThe field policy system handles pagination more effectively than a\nhand-written updateQuery function, and you only need to define the policy\nonce, rather than every time you call fetchMore.");
    ...
    ObservableQuery.prototype.subscribeToMore = function (options) {
        ...
        var subscription = this.queryManager
            ...
            error: function (err) {
                ...
                tsInvariant.invariant.error('Unhandled GraphQL subscription error', err);
            },
        });
...
function defaultSubscriptionObserverErrorCallback(error) {
    tsInvariant.invariant.error('Unhandled error', error.message, error.stack);
}
...
var StoreReader = (function () {
    ...
    StoreReader.prototype.execSelectionSetImpl = function (_a) {
        ...
        if (isReference(objectOrReference) &&
            ...
            return {
                result: {},
                missing: [missingFromInvariant(new tsInvariant.InvariantError("Dangling reference to missing " + objectOrReference.__ref + " object"), context)],
            };
        }
            ...
            if (isField(selection)) {
                ...
                if (fieldValue === void 0) {
                    if (!addTypenameToDocument.added(selection)) {
                        getMissing().push(missingFromInvariant(new tsInvariant.InvariantError("Can't find field '" + selection.name.value + "' on " + (isReference(objectOrReference)
    ...
    if (!field.selectionSet) {
        ...
        workSet_1.forEach(function (value) {
            if (value && typeof value === "object") {
                tsInvariant.invariant(!isReference(value), "Missing selection set for object of type " + getTypenameFromStoreObject(store, value) + " returned for query field " + field.name.value);
...
var StoreWriter = (function () {
    ...
    StoreWriter.prototype.writeToStore = function (_a) {
        ...
        if (!isReference(ref)) {
            throw new tsInvariant.InvariantError("Could not identify object " + JSON.stringify(result));
        }
        ...
        workSet.forEach(function (selection) {
            ...
            if (isField(selection)) {
                ...
                if (typeof value !== 'undefined') {
                    ...
                    throw new tsInvariant.InvariantError("Missing field '" + resultFieldKey + "' in " + JSON.stringify(result, null, 2).substring(0, 100));
                }
            }
            else {
                var fragment = getFragmentFromSelection(selection, context.fragmentMap);
...
function warnAboutDataLoss(existingRef, incomingObj, storeFieldName, store) {
    ...
    tsInvariant.invariant.warn("Cache data may be lost when replacing the " + fieldName + " field of a " + parentType + " object.\n\nTo address this problem (which is not a bug in Apollo Client), " + (childTypenames.length
...
var Policies = (function () {
    ...
    Policies.prototype.setRootTypename = function (which, typename) {
        ...
        if (typename !== old) {
            tsInvariant.invariant(!old || old === which, "Cannot change root " + which + " __typename more than once");
            if (old)
                ...
                if (supertypeSet.has(supertype)) {
                    if (!typenameSupertypeSet.has(supertype)) {
                        if (checkingFuzzySubtypes) {
                            tsInvariant.invariant.warn("Inferring subtype " + typename + " of supertype " + supertype);
                        }
    ...
    return {
        ...
        mergeObjects: function (existing, incoming) {
            if (Array.isArray(existing) || Array.isArray(incoming)) {
                throw new tsInvariant.InvariantError("Cannot automatically merge arrays");
            }
...
function computeKeyObject(response, specifier, aliasMap) {
    ...
    specifier.forEach(function (s) {
        if (Array.isArray(s)) {
            ...
            tsInvariant.invariant(hasOwn.call(response, responseName), "Missing field '" + responseName + "' while computing key fields");
...
var QueryManager = (function () {
    ...
    QueryManager.prototype.stop = function () {
        ...
        this.cancelPendingFetches(new tsInvariant.InvariantError('QueryManager stopped while query was in flight'));
    };
    ...
    QueryManager.prototype.mutate = function (_a) {
        ...
        return tslib.__awaiter(this, void 0, void 0, function () {
            ...
            return tslib.__generator(this, function (_f) {
                switch (_f.label) {
                    case 0:
                        tsInvariant.invariant(mutation, 'mutation option is required. You must specify your GraphQL document in the mutation option.');
                        tsInvariant.invariant(!fetchPolicy || fetchPolicy === 'no-cache', "Mutations only support a 'no-cache' fetchPolicy. If you don't want to disable the cache, remove your fetchPolicy setting to proceed with the default mutation behavior.");
                        ...
                        if (optimisticResponse) {
                            ...
                            this.cache.recordOptimisticTransaction(function (cache) {
                                try {
                                    ...
                                    tsInvariant.invariant.error(error);
                                }
                            }, mutationId);
                        }
    ...
    QueryManager.prototype.query = function (options) {
        var _this = this;
        tsInvariant.invariant(options.query, 'query option is required. You must specify your GraphQL document ' +
            'in the query option.');
        tsInvariant.invariant(options.query.kind === 'Document', 'You must wrap the query string in a "gql" tag.');
        tsInvariant.invariant(!options.returnPartialData, 'returnPartialData option only supported on watchQuery.');
        tsInvariant.invariant(!options.pollInterval, 'pollInterval option only supported on watchQuery.');
    ...
    QueryManager.prototype.clearStore = function () {
        this.cancelPendingFetches(new tsInvariant.InvariantError('Store reset while query was in flight (not completed in link chain)'));
    ...
    QueryManager.prototype.fetchQueryByPolicy = function (queryInfo, options, networkStatus) {
        ...
        var resultsFromCache = function (diff, networkStatus) {
            ...
            if (process.env.NODE_ENV !== 'production' &&
                ...
                !equality.equal(data, {})) {
                tsInvariant.invariant.warn("Missing cache result fields: " + diff.missing.map(function (m) { return m.path.join('.'); }).join(', '), diff.missing);
            }
...
var ApolloClient = (function () {
    function ApolloClient(options) {
        ...
        if (!cache) {
            throw new tsInvariant.InvariantError("To initialize Apollo Client, you must specify a 'cache' property " +
    ...
    ApolloClient.prototype.query = function (options) {
        ...
        tsInvariant.invariant(options.fetchPolicy !== 'cache-and-network', 'The cache-and-network fetchPolicy does not work with client.query, because ' +
...
var ApolloConsumer = function (props) {
    ...
    return (React__default.createElement(ApolloContext.Consumer, null, function (context) {
        tsInvariant.invariant(context && context.client, 'Could not find "client" in the context of ApolloConsumer. ' +
...
var ApolloProvider = function (_a) {
    ...
    return (React__default.createElement(ApolloContext.Consumer, null, function (context) {
        ...
        tsInvariant.invariant(context.client, 'ApolloProvider was not passed a client instance. Make ' +
...
function useApolloClient() {
    ...
    tsInvariant.invariant(client, 'No Apollo Client instance can be found. Please ensure that you ' +
...
function parser(document) {
    ...
    tsInvariant.invariant(!!document && !!document.kind, "Argument of " + document + " passed to parser was not a valid GraphQL " +
    ...
    tsInvariant.invariant(!fragments.length ||
        (queries.length || mutations.length || subscriptions.length), "Passing only a fragment to 'graphql' is not yet supported. " +
        "You must include a query, subscription or mutation as well");
    tsInvariant.invariant(queries.length + mutations.length + subscriptions.length <= 1, "react-apollo only supports a query, subscription, or a mutation per HOC. " +
    ...
    var definitions = queries.length
        ...
        : mutations.length
            ? mutations
            : subscriptions;
    tsInvariant.invariant(definitions.length === 1, "react-apollo only supports one definition per HOC. " + document + " had " +
...
var OperationData = (function () {
    ...
    OperationData.prototype.refreshClient = function () {
        ...
        tsInvariant.invariant(!!client, 'Could not find "client" in the context or passed in as an option. ' +
    ...
    OperationData.prototype.verifyDocumentType = function (document, type) {
        ...
        tsInvariant.invariant(operation.type === type, "Running a " + requiredOperationName + " requires a graphql " +
